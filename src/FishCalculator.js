...
export default class FishCalculator {
  ...
  sortByRarity() {
     ...
     */
    let gatheredFish = Object.fromEntries(
  ...
  #getBucketsByLocation(total) {
    ...
    if (total >= 1) {
      ...
      return {
        confBuckets: total,
        confTime: total * this.minsToCapacity,
        avgBuckets: total,
        avgTime: total * this.minsToCapacity,
      };
    } else if (total <= 0) {
      total = 0;
      return {
  ...
  #getByLocationRarity(location, rarity) {
    ...
    const playground = Object.entries(this.locationInfo).find(([pg, streets]) =>
    ...
    for (let fish of fishList) {
      ...
      for (let [playground, streets] of Object.entries(this.locationInfo)) {
        if (
          playground === location &&
          streets.some((street) => fish.locations.includes(street)) &&
          !gatheredFish.includes(fish)
        ) {
          fishMatch(playground, rarity, fish);
        }
      }
  ...
  #getRarityByLocation(location) {
    ...
    for (let rarity = 1; rarity <= 10; rarity++) {
      ...
      if (rarityFish.length > 0) {
        ...
        const newFish = rarityFish.filter(
  ...
  #getHighestProbability(fish) {
     ...
     */
    return this.#getLocationProbabilities(fish).reduce(
      ...
      if (loc == "Anywhere") {
        if (fish.locations[0] == loc) {
          ...
          for (let entry of probabilities) {
            related = this.#getByLocationRarity(
              entry.location,
              this.#getRarity(fish, loc)
            );
            // related.length is incremented to account for fish not being found
            entry.probability +=
              this.#getRodRarity(fish, loc) / (related.length + 1);
          }
        }
      } else {
        // add base rarity
